...
VALUES ('customer.two', 'customer.two@securefile.test', '$2a$10$YGfslg3v2LQnLWpsr0caOuUeHhiyhg2CV5MBg6NN5RE2zlDzeQ9wK', 'Liam', 'Naidoo',
...
WITH generated_transactions AS (
    SELECT (SELECT id FROM accounts WHERE account_number = '100000000002') AS account_id,
           sequence_number,
           ...
           END AS description,
           CASE
               WHEN sequence_number % 8 = 1 THEN 'SALARY-' || to_char(DATE '2025-07-02' + ((sequence_number - 1) * 356 / 99)::int, 'YYYYMM')
               WHEN sequence_number % 8 = 2 THEN 'BOND-' || to_char(DATE '2025-07-02' + ((sequence_number - 1) * 356 / 99)::int, 'YYYYMM')
               ...
               WHEN sequence_number % 8 = 6 THEN 'MED-' || to_char(DATE '2025-07-02' + ((sequence_number - 1) * 356 / 99)::int, 'YYYYMM')
               WHEN sequence_number % 8 = 7 THEN 'SCHOOL-' || lpad(sequence_number::text, 3, '0')
               ELSE 'REST-' || lpad(sequence_number::text, 3, '0')
           END AS reference,
           CASE
               WHEN sequence_number % 8 = 1 THEN 28750.00
               WHEN sequence_number % 8 = 2 THEN -11200.00
               WHEN sequence_number % 8 = 3 THEN -round((520 + (sequence_number % 13) * 69.35)::numeric, 2)
               WHEN sequence_number % 8 = 4 THEN -round((400 + (sequence_number % 4) * 150)::numeric, 2)
               WHEN sequence_number % 8 = 5 THEN -round((720 + (sequence_number % 8) * 68.90)::numeric, 2)
...
VALUES ((SELECT id FROM customers WHERE customer_number = 'CUST-0002'), (SELECT id FROM accounts WHERE account_number = '100000000002'),
       ...
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0002'), (SELECT id FROM accounts WHERE account_number = '100000000002'),
        '3 Month Statement', '2026-03-01', '2026-05-31', 'statements/CUST-0002/3-month-2026-03-to-2026-05.pdf',
        'CUST-0002-3-month-2026-03-to-2026-05.pdf', 0, 'seed-cust-0002-3-month-2026-03-to-2026-05', '2026-06-01 08:05:00'),
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0002'), (SELECT id FROM accounts WHERE account_number = '100000000002'),
        '6 Month Statement', '2025-12-01', '2026-05-31', 'statements/CUST-0002/6-month-2025-12-to-2026-05.pdf',
        'CUST-0002-6-month-2025-12-to-2026-05.pdf', 0, 'seed-cust-0002-6-month-2025-12-to-2026-05', '2026-06-01 08:05:00'),
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0002'), (SELECT id FROM accounts WHERE account_number = '100000000002'),
        '9 Month Statement', '2025-09-01', '2026-05-31', 'statements/CUST-0002/9-month-2025-09-to-2026-05.pdf',
        'CUST-0002-9-month-2025-09-to-2026-05.pdf', 0, 'seed-cust-0002-9-month-2025-09-to-2026-05', '2026-06-01 08:05:00');
