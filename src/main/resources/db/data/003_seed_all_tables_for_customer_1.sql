...
VALUES ('customer.one', 'customer.one@securefile.test', '$2a$10$YGfslg3v2LQnLWpsr0caOuUeHhiyhg2CV5MBg6NN5RE2zlDzeQ9wK',
        'Ava', 'Mokoena',
...
WITH generated_transactions AS (SELECT (SELECT id FROM accounts WHERE account_number = '100000000001') AS account_id,
                                       sequence_number,
                                       ...
                                       CASE
                                           WHEN sequence_number % 8 = 1 THEN 'SALARY-' || to_char(
                                                   DATE '2025-07-01' + ((sequence_number - 1) * 357 / 99)::int,
                                                   'YYYYMM')
                                           WHEN sequence_number % 8 = 2 THEN 'RENT-' || to_char(
                                                   DATE '2025-07-01' + ((sequence_number - 1) * 357 / 99)::int,
                                                   'YYYYMM')
                                           ...
                                           WHEN sequence_number % 8 = 6 THEN 'INS-' || to_char(
                                                   DATE '2025-07-01' + ((sequence_number - 1) * 357 / 99)::int,
                                                   'YYYYMM')
                                           WHEN sequence_number % 8 = 7
                                               THEN 'SUB-' || lpad(sequence_number::text, 3, '0')
                                       ...
                                       CASE
                                           ...
                                           WHEN sequence_number % 8 = 3
                                               THEN -round((450 + (sequence_number % 11) * 83.45)::numeric, 2)
                                           WHEN sequence_number % 8 = 4
                                               THEN -round((300 + (sequence_number % 5) * 100)::numeric, 2)
                                           WHEN sequence_number % 8 = 5
                                               THEN -round((650 + (sequence_number % 7) * 75.30)::numeric, 2)
...
VALUES ((SELECT id FROM customers WHERE customer_number = 'CUST-0001'),
        (SELECT id FROM accounts WHERE account_number = '100000000001'),
       ...
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0001'),
        ...
        'CUST-0001-3-month-2026-03-to-2026-05.pdf', 0, 'seed-cust-0001-3-month-2026-03-to-2026-05',
        '2026-06-01 08:00:00'),
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0001'),
        (SELECT id FROM accounts WHERE account_number = '100000000001'),
        '6 Month Statement', '2025-12-01', '2026-05-31', 'statements/CUST-0001/6-month-2025-12-to-2026-05.pdf',
        'CUST-0001-6-month-2025-12-to-2026-05.pdf', 0, 'seed-cust-0001-6-month-2025-12-to-2026-05',
        '2026-06-01 08:00:00'),
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0001'),
        (SELECT id FROM accounts WHERE account_number = '100000000001'),
        '9 Month Statement', '2025-09-01', '2026-05-31', 'statements/CUST-0001/9-month-2025-09-to-2026-05.pdf',
        'CUST-0001-9-month-2025-09-to-2026-05.pdf', 0, 'seed-cust-0001-9-month-2025-09-to-2026-05',
        '2026-06-01 08:00:00');
