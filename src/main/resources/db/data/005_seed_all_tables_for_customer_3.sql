...
VALUES ('customer.three', 'customer.three@securefile.test', '$2a$10$YGfslg3v2LQnLWpsr0caOuUeHhiyhg2CV5MBg6NN5RE2zlDzeQ9wK', 'Mia',
...
WITH generated_transactions AS (
    SELECT (SELECT id FROM accounts WHERE account_number = '100000000003') AS account_id,
           sequence_number,
           ...
           END AS description,
           CASE
               WHEN sequence_number % 8 = 1 THEN 'SALARY-' || to_char(DATE '2025-07-03' + ((sequence_number - 1) * 355 / 99)::int, 'YYYYMM')
               WHEN sequence_number % 8 = 2 THEN 'VEHICLE-' || to_char(DATE '2025-07-03' + ((sequence_number - 1) * 355 / 99)::int, 'YYYYMM')
               ...
               WHEN sequence_number % 8 = 6 THEN 'CELL-' || to_char(DATE '2025-07-03' + ((sequence_number - 1) * 355 / 99)::int, 'YYYYMM')
               WHEN sequence_number % 8 = 7 THEN 'ELEC-' || lpad(sequence_number::text, 3, '0')
               ELSE 'ONLINE-' || lpad(sequence_number::text, 3, '0')
           END AS reference,
           CASE
               WHEN sequence_number % 8 = 1 THEN 36100.00
               WHEN sequence_number % 8 = 2 THEN -6400.00
               WHEN sequence_number % 8 = 3 THEN -round((390 + (sequence_number % 12) * 74.80)::numeric, 2)
               WHEN sequence_number % 8 = 4 THEN -round((250 + (sequence_number % 6) * 100)::numeric, 2)
               WHEN sequence_number % 8 = 5 THEN -round((580 + (sequence_number % 9) * 61.40)::numeric, 2)
               WHEN sequence_number % 8 = 6 THEN -899.00
               WHEN sequence_number % 8 = 7 THEN -round((350 + (sequence_number % 5) * 80)::numeric, 2)
...
VALUES ((SELECT id FROM customers WHERE customer_number = 'CUST-0003'), (SELECT id FROM accounts WHERE account_number = '100000000003'),
       ...
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0003'), (SELECT id FROM accounts WHERE account_number = '100000000003'),
        '3 Month Statement', '2026-03-01', '2026-05-31', 'statements/CUST-0003/3-month-2026-03-to-2026-05.pdf',
        'CUST-0003-3-month-2026-03-to-2026-05.pdf', 0, 'seed-cust-0003-3-month-2026-03-to-2026-05', '2026-06-01 08:10:00'),
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0003'), (SELECT id FROM accounts WHERE account_number = '100000000003'),
        '6 Month Statement', '2025-12-01', '2026-05-31', 'statements/CUST-0003/6-month-2025-12-to-2026-05.pdf',
        'CUST-0003-6-month-2025-12-to-2026-05.pdf', 0, 'seed-cust-0003-6-month-2025-12-to-2026-05', '2026-06-01 08:10:00'),
       ((SELECT id FROM customers WHERE customer_number = 'CUST-0003'), (SELECT id FROM accounts WHERE account_number = '100000000003'),
        '9 Month Statement', '2025-09-01', '2026-05-31', 'statements/CUST-0003/9-month-2025-09-to-2026-05.pdf',
        'CUST-0003-9-month-2025-09-to-2026-05.pdf', 0, 'seed-cust-0003-9-month-2025-09-to-2026-05', '2026-06-01 08:10:00');
